    ...
    let
      ...
      final = {
        ...
        emulator = pkgs:
          ...
          && pkgs.stdenv.hostPlatform.isCompatible final then
            ''${pkgs.runtimeShell} -c '"$@"' --''
          else if final.isWindows then
            "${wine}/bin/${wine-name}"
          else if final.isLinux && pkgs.stdenv.hostPlatform.isLinux then
      ...
      } // mapAttrs (n: v: v final.parsed) inspect.predicates // args;
