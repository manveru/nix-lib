...
let
  ...
  outer_types = rec {
    ...
    types = rec {
      unspecified = mkOptionType { name = "unspecified"; };
      ...
      list =
        builtins.trace "`types.list` is deprecated; use `types.listOf` instead"
        types.listOf;
      ...
      loaOf = elemType:
        let
          convertAllLists = loc: defs:
            ...
            in imap1 (i: convertIfList loc (unnamedPrefix i)) defs;
            ...
            if isList def.value then
              let
                ...
                matched = let
                  ...
                  fallback = { name = "name"; };
                in findFirst ({ path, ... }:
                  all (v: v == true) (zipListsWith equals loc path)) fallback
                nameAttrs;
                ...
                nameValueNew = value: unnamed:
                  if isList value then
                    if length value > 0 then head value else unnamed
                  else
                    value;
                res = {
                  inherit (def) file;
                  value = listToAttrs (imap1 (elemIdx: elem: {
                    name = nameValueNew (elem.${nameAttr} or (unnamed elemIdx))
                      (unnamed elemIdx);
                    value = elem;
                  }) def.value);
                };
                ...
                list = concatMapStrings (x:
                  "{ ${nameAttr} = ${
                    nameValueOld (x.${nameAttr} or "unnamed")
                  }; ...} ") sample;
                set = concatMapStrings (x:
                  "${
                    nameValueNew (x.${nameAttr} or "unnamed") "unnamed"
                  } = {...}; ") sample;
                msg = ''
                  In file ${def.file}
      ...
      submoduleWith = { modules, specialArgs ? { }
        , shorthandOnlyDefinesConfig ? false }@attrs:
        let
          inherit (lib.modules) evalModules;

          coerce = unify: value:
            if isFunction value then
              setFunctionArgs (args: unify (value args)) (functionArgs value)
            else
              unify (if shorthandOnlyDefinesConfig then {
                config = value;
              } else
                value);
          ...
          allModules = defs:
            modules ++ imap1 (n:
              ...
              # Annotate the value with the location of its definition for better error messages
                coerce (lib.modules.unifyModuleSyntax file
                  "${toString file}-${toString n}") value
              else
                value) defs;
        ...
        in mkOptionType rec {
          ...
          functor = defaultFunctor name // {
            ...
            binOp = lhs: rhs: {
              ...
              shorthandOnlyDefinesConfig = if lhs.shorthandOnlyDefinesConfig
      ...
      coercedTo = coercedType: coerceFunc: finalType:
        ...
        mkOptionType rec {
          name = "coercedTo";
          description =
            "${finalType.description} or ${coercedType.description} convertible to it";
          check = x:
