...
rec {
  ...
  pathIsDirectory = p:
    if builtins.pathExists p then (pathType p) == "directory" else false;

  # Returns true if the path exists and is a regular file, false otherwise
  pathIsRegularFile = p:
    if builtins.pathExists p then (pathType p) == "regular" else false;
  ...
  cleanSourceWith = { filter ? _path: _type: true, src, name ? null }:
    let
      ...
      filter' = if isFiltered then
        name: type: filter name type && src.filter name type
      else
        filter;
  ...
  commitIdFromGitRepo = let
    readCommitFromFile = file: path:
      ...
      then
        ...
        in if m == null then
          throw ("File contains no gitdir reference: " + path)
        else
          let
